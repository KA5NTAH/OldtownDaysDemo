...
class Channel:
    ...
    def create_and_set_ball(self, metal):
        ...
        drop_x = (
            self.channel_rect.left + half_width - int(game_constants.DROPLET_WIDTH / 2)
        )
    ...
    def update(self, link_fills_per_drop=1, mode="normal"):
        ...
        for index in range(len(self._droplets)):
            ...
            intersection = utils.get_intersection(
                self._link_rect, self._droplets[index].rect
            )
        ...
        self._droplets = [
            self._droplets[i]
            for i in range(len(self._droplets))
            if i not in droplets_ixs_to_discard
        ]
...
if __name__ == "__main__":
    ...
    link = Link(
        empty, full, timer, (40, 533), 5000, Metals.GOLD, game_constants.MOUSE_KEY
    )
    p = Channel(link)
