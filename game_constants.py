...
LVL_EVENTS_TYPES = (
    (GENERATE_COIN_EVENT.type, GENERATE_DROP_EVENT.type, RUINED_DROP_EVENT.type)
    + LINK_IS_DONE_EVENTS_TYPES
    + NO_LINK_RUIN_TYPES
)

# backgrounds images
LVL_BACKGROUND = pygame.image.load(
    os.path.join(BACKGROUNDS_DIR, "Level_background.png")
)
...
MENU_BACKGROUND = pygame.image.load(
    os.path.join(BACKGROUNDS_DIR, "Menu_background.png")
)
MENU_BUTTONS_ORDER = [
    GameState.GAME_MODE_CHOOSING,
    GameState.EDUCATION,
...
buttons_y = (
    np.arange(len(MENU_BUTTONS_ORDER)) * (MENU_BUTTON_H + MENU_BUTTON_Y_GAP)
    + MENU_BUTTON_Y_OFFSET
)
...
for state, pos in zip(MENU_BUTTONS_ORDER, MENU_BUTTONS_POSITIONS):
    ...
    button_info = {
        "images": [idle_image, addressing_image],
        "position": pos,
        "state": state,
    }
...
# education images
EDUCATION_IMAGES = [
    pygame.image.load(os.path.join(EDUCATION_DIR, curr_name))
...
EDUCATIONAL_PROGRESS_LENGTH = (
    EDUCATION_SLIDES_AMOUNT * EDUCATIONAL_PROGRESS_RAD * 2
    + EDUCATIONAL_PROGRESS_X_GAP * (EDUCATION_SLIDES_AMOUNT - 1)
)
EDUCATIONAL_PROGRESS_X_START = int((1200 - EDUCATIONAL_PROGRESS_LENGTH) / 2)
EDUCATIONAL_PROGRESS_COORD = [
    [
        EDUCATIONAL_PROGRESS_X_START
        + EDUCATIONAL_PROGRESS_RAD * 2 * i
        + EDUCATIONAL_PROGRESS_X_GAP * i,
        EDUCATIONAL_PROGRESS_Y,
    ]
    for i in range(EDUCATION_SLIDES_AMOUNT)
]
...
ACHIEVEMENTS_BACKGROUND = pygame.image.load(
    os.path.join(BACKGROUNDS_DIR, "Achievements_background.png")
)
...
x = np.arange(
    ACHIEVEMENT_X_START, ACHIEVEMENT_X_STOP + 1, ACH_WIDTH + ACHIEVEMENT_X_GAP
)
y = np.arange(
    ACHIEVEMENT_Y_START, ACHIEVEMENT_Y_STOP + 1, ACH_HEIGHT + ACHIEVEMENT_Y_GAP
)
...
for achievement in AchievementsNames:
    ...
    if os.path.exists(ach_dir):
        ...
        ACHIEVEMENTS_IMAGES[achievement] = [
            small_icon_locked,
            small_icon_unlocked,
            big_icon_locked,
            big_icon_unlocked,
...
MODE_SELECTION_BACKGROUND = pygame.image.load(
    os.path.join(BACKGROUNDS_DIR, "Mode_choosing_background.png")
)
...
for state, pos in zip(
    [GameState.INFINITE_PLAY, GameState.LEVEL_CHOOSING], selection_positions
):
    ...
    MODE_SELECTION_BUTTONS_INFO.append(
        {"images": images, "position": pos, "state": state}
    )
...
LVL_SHOOSING_BACKGROUND = pygame.image.load(
    os.path.join(BACKGROUNDS_DIR, "Level_choosing_background.png")
)
...
x = np.arange(
    LVL_BUTTONS_X_START, LVL_BUTTONS_X_STOP + 1, LVL_ICON_WIDTH + LVL_BUTTONS_X_GAP
)
y = np.arange(
    LVL_BUTTONS_Y_START, LVL_BUTTONS_Y_STOP + 1, LVL_ICON_HEIGHT + LVL_BUTTONS_Y_GAP
)
xx, yy = np.meshgrid(x, y)
...
LVL_BUTTONS_INFO = []
LOCKED_LEVEL_IMAGE = pygame.image.load(
    os.path.join(LVL_BUTTONS_DIR, "LvlCloseIcon.png")
)
for folder_name, pos in zip(sorted(os.listdir(LVL_BUTTONS_DIR)), LVL_BUTTONS_POSITIONS):
    ...
    LVL_BUTTONS_INFO.append(
        {
            "images": [idle_image, selected_image],
            "position": pos,
            "lvl_num": int(folder_name),
        }
    )
...
STRANGER_CHALLENGE_BG = pygame.image.load(
    os.path.join(CHALLENGE_BACKGROUNDS_FOLDER, "StrangerBG.png")
)
STRANGER_CHALLENGE_COORDIANTES = [
    [577, 148],
    [690, 154],
    [462, 308],
    [766, 318],
...
CHALLENGE_BG = pygame.image.load(
    os.path.join(CHALLENGE_BACKGROUNDS_FOLDER, "ChallengeBG.png")
)
RGB_METALS_COLORS = {
    Metals.GOLD: (255, 215, 0),
...
for metal in Metals:
    ...
    targets_sub_folders = sorted(
        os.path.join(challenge_sub_folder, f) for f in os.listdir(challenge_sub_folder)
    )
    ...
    for target_folder in targets_sub_folders:
        ...
        curr_images = [
            pygame.image.load(trace_image_path),
...
for metal in Metals:
    ...
    LINKS_IMAGES[metal] = [
        pygame.image.load(empty_img_path),
...
five_links_arrangement = np.vstack((five_links_x, np.ones((5,)) * links_y)).T.astype(
    np.int32
)
...
for coin_kind in CoinsKinds:
    COINS_IMAGES[coin_kind] = pygame.image.load(
        os.path.join(COINS_FOLDER, f"{coin_kind.name}.png")
    )
...
LOSER_OPTIONS_BACKGROUND = pygame.image.load(
    os.path.join(BACKGROUNDS_DIR, "Loser_options_background.png")
)
loser_buttons_positions = {"bribe": [60, 80], "menu": [443, 80], "trial": [826, 80]}
BRIBE_COST = 250
...
for folder in os.listdir(loser_options_dir):
    if folder != "menu":
        idle_img = pygame.image.load(
            os.path.join(loser_options_dir, folder, "idle.png")
        )
        closed_img = pygame.image.load(
            os.path.join(loser_options_dir, folder, "closed.png")
        )
        opened_img = pygame.image.load(
            os.path.join(loser_options_dir, folder, "opened.png")
        )
        ...
        addressing_img = pygame.image.load(
            os.path.join(loser_options_dir, folder, "addressing.png")
        )
        images = [idle_img, addressing_img]
    LOSER_BUTTONS_INFO[folder] = {
        "images": images,
        "position": loser_buttons_positions[folder],
    }
...
WINNER_OPTIONS_BACKGROUND = pygame.image.load(
    os.path.join(BACKGROUNDS_DIR, "Winner_options_background.png")
)
back_to_level_images = [
    pygame.image.load(os.path.join(WINNER_OPTIONS_FOLDER, "back_to_levels", img_name))
    for img_name in ("idle.png", "addressing.png")
]
next_level_images = [
    pygame.image.load(os.path.join(WINNER_OPTIONS_FOLDER, "next_level", img_name))
    for img_name in ("idle.png", "addressing.png")
]
WINNER_OPTIONS_INFO = {
    "back_to_menu": {"images": back_to_level_images, "position": [227, 80]},
    "next_level": {"images": next_level_images, "position": [659, 80]},
}
...
for bar_name in ("winning", "losing"):
    ...
    PROGRESS_BAR_IMAGES[bar_name] = [
        pygame.image.load(os.path.join(bar_folder, "background.png")),
...
EMPTY_LINK_FILLER = pygame.image.load(
    os.path.join(GENERAL_FOLDER, "empty_link_filler.png")
)
...
for god in Bonuses:
    if god == Bonuses.FATHER:
        TRIAL_BACKGROUNDS[god] = {
            "positive": pygame.image.load(
                os.path.join(TRIAL_BG_FOLDER, f"{god.name}_POS.png")
            ),
            "negative": pygame.image.load(
                os.path.join(TRIAL_BG_FOLDER, f"{god.name}_NEG.png")
            ),
        }
    else:
        TRIAL_BACKGROUNDS[god] = pygame.image.load(
            os.path.join(TRIAL_BG_FOLDER, f"{god.name}.png")
        )

TRIAL_BUTTON_POSITION = (60, 574)
TRIAL_BUTTON_FOLDER = os.path.join(RESOURSES_DIR, "TrialButton")
TRIAL_BUTTON_IMAGES = [
    pygame.image.load(os.path.join(TRIAL_BUTTON_FOLDER, "idle.png")),
    pygame.image.load(os.path.join(TRIAL_BUTTON_FOLDER, "addressing.png")),
]

# Currencies Manager
TARG_COIN_MINIATURE = pygame.image.load(
    os.path.join(CURRENCIES_MANAGER_DIR, "targ_miniature.png")
)
FAITH_COIN_MINIATURE = pygame.image.load(
    os.path.join(CURRENCIES_MANAGER_DIR, "faith_miniature.png")
)
CURRENCIES_MANAGER_BG = pygame.image.load(
    os.path.join(CURRENCIES_MANAGER_DIR, "bg.png")
)


# Bonuses Icons
BONUS_MENU_COORD = (975, 247)
BONUSES_ICONS_IMAGES = {
    None: pygame.image.load(os.path.join(BONUSES_ICONS_DIR, "BLANK.png"))
}
for bonus in Bonuses:
...
BONUSES_LVL_ICONS_IMAGES = {
    None: pygame.image.load(os.path.join(BONUSES_LVL_ICONS_DIR, "BLANK.png"))
}
for bonus in Bonuses:
